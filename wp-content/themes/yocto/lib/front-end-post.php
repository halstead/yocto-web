...
if (!class_exists("DjdSitePost")) {
	...
	public function register_plugin_scripts() {
		wp_enqueue_script( 'google-maps-api-script', 'http://maps.google.com/maps/api/js?sensor=false'  );
		wp_enqueue_script( 'djd-site-post-script', plugins_url( 'djd-site-post/js/display.js' ), array( 'jquery' ) );
	...
	function process_site_post_form() {
		if( isset($_POST) ){
			...
			//$postdate = date('2015-08-23 18:57:33');
			//=> date("Y-m-d H:i:s", strtotime("Sept 11, 2001"))
			
			//$date_string = "2015-08-04"; // or any string like "20110911" or "2011-09-11"
			...
			if ( !empty ($_POST["djd-our-id"])) $djd_post_id = $_POST["djd-our-id"];
				...
				$my_post = array(
					'ID' => $djd_post_id,
					'edit_date' => 'true', /* Had to be added to make date work*/
					'post_date' => '',  /* Custom Date */
					'post_title' => '',
					'post_status' => 'publish',
					'post_author' => '',
					'post_category' => '',
					...
					'tags_input' => '',
					'to_ping' =>  ''
					//'my_meta_box_city_text' =>  'birmingham'
				);
...
// 				
				...
				if( array_key_exists('djd_site_post_date', $_POST)) {
					//$my_post['tags_input'] = wp_strip_all_tags($_POST['djd_site_post_tags']);
					//$my_post['post_date'] = wp_strip_all_tags($_POST['djd_site_post_date']); /* Custom Date */
					//$my_post['post_date'] = '2015-08-04'; //wp_strip_all_tags($_POST['djd_site_post_date']); // 09:10:22  '2015-08-04'; //
					//date("Y-m-d H:i:s", strtotime("Sept 11, 2001"));
					$my_post['post_date'] = date("Y-m-d H:i:s", strtotime($_POST['djd_site_post_date']));
				}
				...
				// Insert meta fields into the post 
				if($post_success != 0)
				{
